document.addEventListener("DOMContentLoaded", async () => {
  ...
  // 🔹 NEUE METHODE: Account type aus profiles Tabelle laden
  async function getUserAccountType() {
    try {
      ...
      if (authError || !user) {
        console.log("No user logged in, using guest menu");
        return "guest";
      }
      ...
      if (profileError) {
        ...
        return "normal"; // Fallback zu normal bei Fehler
      }
      ...
      console.log(`✅ Account type from profiles: ${accountType}`);
      ...
      if (!validTypes.includes(cleanType)) {
        console.warn(
          `⚠️ Invalid account_type: "${cleanType}", defaulting to "normal"`
        );
        return "normal";
      }
  ...
  const menuConfigs = {
    ...
    business: [
      ...
      {
        href: "/src/pages/businessShopEinstellungen.html",
    ...
    admin: [
      ...
      {
        href: "/src/pages/admin/tuninghubAdminNotifications.html",
        text: "System-Benachrichtigungen",
      },
      { href: "/src/pages/teilesuchen.html", text: "Teile suchen" },
  ...
  function createMenuItems(config) {
    ...
    config.forEach((item) => {
      if (item.type === "divider") {
        ...
        menuItem.className =
          "menu-item" + (item.highlight ? " menu-item-highlight" : "");
...
// Utility function to get account type (exportable)
